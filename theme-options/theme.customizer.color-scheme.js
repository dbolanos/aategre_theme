/* global saveo_color_schemes, saveo_dependencies, Color */
...
( function( api ) {
	...
	for (var i in saveo_color_schemes) {
		cssTemplate[i] = wp.template( 'saveo-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'saveo-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+saveo_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(saveo_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', saveo_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', saveo_customizer_actions);

		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			saveo_customizer_check_dependencies(jQuery(this));
		});

		// Init color controls
		jQuery('#customize-theme-controls .saveo_scheme_editor_selector').on('change', function(e) {
			saveo_customizer_change_color_scheme(jQuery(this).val());
		});
		jQuery('#customize-theme-controls .saveo_scheme_editor_colors input').on('change', function(e) {
			saveo_customizer_update_color_scheme(jQuery(this).attr('name'), jQuery(this).val());
		});
		// Color picker
		saveo_color_picker();
		jQuery('#customize-theme-controls .iColorPicker').each(function() {
			var clr = jQuery(this).val();
			saveo_customizer_change_field_colors(jQuery(this), clr);
		});
		jQuery('#customize-theme-controls').on('focus', '.iColorPicker', function (e) {
			saveo_color_picker_show(null, jQuery(this), function(fld, clr) {
				saveo_customizer_change_field_colors(fld, clr);
	...
	api.bind('change', function(obj) {
		...
		saveo_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		saveo_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function saveo_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			var id = jQuery(this).attr('id');
			if (id == undefined) return;
			id = id.replace('customize-control-', '');
			var depend = false;
			for (var fld in saveo_dependencies) {
				if (fld == id) {
					depend = saveo_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('saveo_options_no_use');
				} else {
					jQuery(this).hide().addClass('saveo_options_no_use');
				}
			}
		});
	}
	...
	function saveo_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in saveo_theme_fonts) {
					for (rule in saveo_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							saveo_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) saveo_customizer_update_css();
		}
	...
	// Actions buttons
	function saveo_customizer_actions(e) {
	...
	function saveo_customizer_change_color_scheme(value) {
		updateCSS = false;
		for (var opt in saveo_color_schemes[value].colors) {
			var fld = jQuery('#customize-theme-controls .saveo_scheme_editor_colors input[name="'+opt+'"]');
			if (fld.length == 0) continue;
			fld.val( saveo_color_schemes[value].colors[opt] );
			saveo_customizer_change_field_colors(fld, saveo_color_schemes[value].colors[opt]);
		}
		updateCSS = true;
		saveo_customizer_update_css();
	}

	// Store new value in the color table
	function saveo_customizer_update_color_scheme(opt, value) {
		saveo_color_schemes[jQuery('.saveo_scheme_editor_selector').val()].colors[opt] = value;
		api('scheme_storage').set(saveo_serialize(saveo_color_schemes))
	}
	
	// Change color in the field
	function saveo_customizer_change_field_colors(fld, clr) {
		var hsb = saveo_hex2hsb(clr);
		fld.css({
			'backgroundColor': clr,
	...
	function saveo_customizer_update_theme_fonts(opt, rule, value) {
		saveo_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function saveo_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=saveo_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in saveo_theme_fonts) {
	...
	function saveo_customizer_update_css() {
		...
		var fonts = saveo_customizer_add_theme_fonts(saveo_theme_fonts);

		// Make styles and add into css
		css += saveo_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in saveo_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in saveo_color_schemes[scheme].colors) {
				colors[i] = saveo_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = saveo_customizer_add_theme_colors(colors);
	...
	function saveo_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + saveo_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + saveo_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + saveo_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function saveo_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function saveo_customizer_prepare_html_value(val) {
