...
if (!function_exists('saveo_array_get_first')) {
	function saveo_array_get_first(&$arr, $key=true) {
...
if (!function_exists('saveo_array_from_list')) {
	function saveo_array_from_list($arr) {
		$new = array();
...
// $d = saveo_array_merge($a, $b);	["four", "k2"=>"six", "seven", "k1"=>"five");
if (!function_exists('saveo_array_merge')) {
	function saveo_array_merge($a1, $a2) {
...
if (!function_exists('saveo_array_insert')) {
	function saveo_array_insert_after(&$haystack, $needle, $stuff){
		...
		for ($i = 2; $i < func_num_args(); ++$i){
			...
			if (is_array($arg)) {
				if ($i==2)
					...
					$new_array = saveo_array_merge($new_array, $arg);
			} else 
				$new_array[] = $arg;
		}
		...
		$haystack = saveo_array_merge(array_slice($haystack, 0, $i, true), $new_array, array_slice($haystack, $i, null, true));
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if (!function_exists('saveo_array_before')) {
	function saveo_array_insert_before(&$haystack, $needle, $stuff){
		if (! is_array($haystack) ) return -1;
...
if (!function_exists('saveo_get_tag')) {
	function saveo_get_tag($text, $tag_start, $tag_end) {
		$val = '';
...
if (!function_exists('saveo_get_tag_attrib')) {
	function saveo_get_tag_attrib($text, $tag, $attr) {
		$val = '';
...
if (!function_exists('saveo_html_decode')) {
	function saveo_html_decode($prm) {
...
if (!function_exists('saveo_get_css_position_from_values')) {
	function saveo_get_css_position_from_values($top='',$right='',$bottom='',$left='',$width='',$height='') {
		if (!is_array($top)) {
		...
		foreach ($top as $k=>$v) {
			$imp = substr($v, 0, 1);
			if ($imp == '!') $v = substr($v, 1);
			if ($v != '') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.esc_attr($k))) . ':' . esc_attr(saveo_prepare_css_value($v)) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}
...
if (!function_exists('saveo_prepare_css_value')) {
	function saveo_prepare_css_value($val) {
		if ($val != '') {
			$ed = substr($val, -1);
			if ('0'<=$ed && $ed<='9') $val .= 'px';
		}
...
if (!function_exists('saveo_parse_icons_classes')) {
	function saveo_parse_icons_classes($css) {
		$rez = array();
...
if (!function_exists('saveo_stripslashes')) {
	function saveo_stripslashes($val) {
		static $magic = 0;
		if ($magic === 0) {
...
// Get GET, POST value
if (!function_exists('saveo_get_value_gp')) {
	function saveo_get_value_gp($name, $defa='') {
		if (isset($_GET[$name]))		$rez = $_GET[$name];
		else if (isset($_POST[$name]))	$rez = $_POST[$name];
		else							$rez = $defa;
		return saveo_stripslashes($rez);
	}
}

// Get GET, POST, COOKIE value and save it (if need)
if (!function_exists('saveo_get_value_gpc')) {
	function saveo_get_value_gpc($name, $defa='', $page='', $exp=0) {
		if (isset($_GET[$name]))		 $rez = $_GET[$name];
		else if (isset($_POST[$name]))	 $rez = $_POST[$name];
...
if (!function_exists('saveo_get_value_gps')) {
	function saveo_get_value_gps($name, $defa='', $page='') {
		if (isset($_GET[$name]))		  $rez = $_GET[$name];
		else if (isset($_POST[$name]))	  $rez = $_POST[$name];
...
if (!function_exists('saveo_set_session_value')) {
	function saveo_set_session_value($name, $value, $page='') {
...
// Save value into session
if (!function_exists('saveo_del_session_value')) {
	function saveo_del_session_value($name, $page='') {
		if (!session_id()) session_start();
...
if (!function_exists('saveo_hex2rgb')) {
	function saveo_hex2rgb($hex) {
		$dec = hexdec(substr($hex, 0, 1)== '#' ? substr($hex, 1) : $hex);
		return array('r'=> $dec >> 16, 'g'=> ($dec & 0x00FF00) >> 8, 'b'=> $dec & 0x0000FF);
	}
}

if (!function_exists('saveo_hex2rgba')) {
	function saveo_hex2rgba($hex, $alpha) {
		$rgb = saveo_hex2rgb($hex);
		return 'rgba('.intval($rgb['r']).','.intval($rgb['g']).','.intval($rgb['b']).','.floatval($alpha).')';
	}
}

if (!function_exists('saveo_hex2hsb')) {
	function saveo_hex2hsb($hex, $h=0, $s=0, $b=0) {
		$hsb = saveo_rgb2hsb(saveo_hex2rgb($hex));
...
if (!function_exists('saveo_rgb2hsb')) {
	function saveo_rgb2hsb($rgb) {
		$hsb = array();
...
if (!function_exists('saveo_hsb2rgb')) {
	function saveo_hsb2rgb($hsb) {
		$rgb = array();
		$h = round($hsb['h']);
...
if (!function_exists('saveo_rgb2hex')) {
	function saveo_rgb2hex($rgb) {
		$hex = array(
			dechex($rgb['r']),
			dechex($rgb['g']),
			dechex($rgb['b'])
...
if (!function_exists('saveo_hsb2hex')) {
	function saveo_hsb2hex($hsb) {
		return saveo_rgb2hex(saveo_hsb2rgb($hsb));
	}
}
...
if (!function_exists('saveo_prepare_macros')) {
	function saveo_prepare_macros($str) {
		return str_replace(
			array("{{",  "}}",   "[[",  "]]",   "||"),
			array("<i>", "</i>", "<b>", "</b>", "<br>"),
			$str);
...
if (!function_exists('saveo_remove_macros')) {
	function saveo_remove_macros($str) {
		return str_replace(
			array("{{", "}}", "[[", "]]", "||"),
			array("",   "",   "",   "",   " "),
			$str);
...
if (!function_exists('saveo_is_on')) {
	function saveo_is_on($prm) {
		return $prm>0 || in_array(strtolower($prm), array('true', 'on', 'yes', 'show'));
	}
}
if (!function_exists('saveo_is_off')) {
	function saveo_is_off($prm) {
		return empty($prm) || $prm===0 || in_array(strtolower($prm), array('false', 'off', 'no', 'none', 'hide'));
	}
}
if (!function_exists('saveo_is_inherit')) {
	function saveo_is_inherit($prm) {
		return in_array(strtolower($prm), array('inherit'));		//array('inherit', 'default')
	}
}

// Return truncated string
if (!function_exists('saveo_strshort')) {
	function saveo_strshort($str, $maxlength, $add='...') {
...
if (!function_exists('saveo_unserialize')) {
	function saveo_unserialize($str) {
...
if (!function_exists('saveo_getimagesize')) {
	function saveo_getimagesize($url) {
		...
		$url = saveo_remove_protocol_from_url($url);
		
		foreach ($locations as $key=>$loc) {

			$loc['url'] = saveo_remove_protocol_from_url($loc['url']);
...
if (!function_exists('saveo_clear_thumb_size')) {
	function saveo_clear_thumb_size($url) {
		$pi = pathinfo($url);
...
if (!function_exists('saveo_add_thumb_size')) {
	function saveo_add_thumb_size($url, $thumb_size, $check_exists=true) {
		$pi = pathinfo($url);
		$parts = explode('-', $pi['filename']);
...
if (!function_exists('saveo_get_thumb_size')) {
	function saveo_get_thumb_size($ts) {
		static $retina = '-';
		if ($retina=='-') $retina = saveo_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ($ts=='post-thumbnail' ? '' : 'saveo-thumb-') . $ts . $retina;
	}
}


// Return url from first <img> tag inserted in post
if (!function_exists('saveo_get_post_image')) {
	function saveo_get_post_image($post_text='', $src=true) {
		global $post;
		$img = '';
...
if (!function_exists('saveo_get_post_audio')) {
	function saveo_get_post_audio($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = saveo_get_tag($post_text, '<audio', '</audio>');
			if (empty($img))
				$img = do_shortcode(saveo_get_tag($post_text, '[audio', '[/audio]'));
			if (empty($img)) {
				$img = saveo_get_tag_attrib($post_text, '[trx_widget_audio]', 'url');
				if (!empty($img))
...
if (!function_exists('saveo_get_post_video')) {
	function saveo_get_post_video($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = saveo_get_tag($post_text, '<video', '</video>');
			if (empty($img)) {
				$img = do_shortcode(saveo_get_tag($post_text, '[video', '[/video]'));
			}
		}
		return $img;
	}
...
if (!function_exists('saveo_get_post_iframe')) {
	function saveo_get_post_iframe($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = saveo_get_tag($post_text, '<iframe', '</iframe>');
		return $img;
	}
}

// Add 'autoplay' feature in the video
if (!function_exists('saveo_make_video_autoplay')) {
	function saveo_make_video_autoplay($video) {
...
if (!function_exists('saveo_is_from_uploads')) {
	function saveo_is_from_uploads($url) {
		$local = false;
		$uploads_info = wp_upload_dir();
		$uploads_url = $uploads_info['baseurl'];
		$uploads_dir = $uploads_info['basedir'];
...
if (!function_exists('saveo_is_youtube_url')) {
	function saveo_is_youtube_url($url) {
		return strpos($url, 'youtu.be')!==false || strpos($url, 'youtube.com')!==false;
	}
}

// Check if URL from Vimeo
if (!function_exists('saveo_is_vimeo_url')) {
	function saveo_is_vimeo_url($url) {
...
------------------------------------------------------------------- */
if (!function_exists('saveo_init_filesystem')) {
	add_action( 'after_setup_theme', 'saveo_init_filesystem', 0);
	function saveo_init_filesystem() {
...
if (!function_exists('saveo_fpc')) {	
	function saveo_fpc($file, $data, $flag=0) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (saveo_is_on(saveo_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'saveo'), $file));
			}
		}
		return false;
	}
...
if (!function_exists('saveo_fgc')) {	
	function saveo_fgc($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (saveo_is_on(saveo_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'saveo'), $file));
			}
		}
		return '';
	}
...
if (!function_exists('saveo_fga')) {	
	function saveo_fga($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (saveo_is_on(saveo_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'saveo'), $file));
			}
		}
		return array();
	}
...
if (!function_exists('saveo_esc')) {	
	function saveo_esc($file) {
...
if (!function_exists('saveo_check_min_file')) {	
	function saveo_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && saveo_is_off(saveo_get_theme_option('debug_mode'))) {
			...
			if (substr($file, -8)!='.min.css'  && saveo_is_off(saveo_get_theme_option('debug_mode'))) {
				$dir = trailingslashit($dir);
...
if (!function_exists('saveo_get_file_dir')) {	
	function saveo_get_file_dir($file, $return_url=false) {
		...
		if (file_exists(($child_dir).'/'.($file)))
			$dir = ($return_url ? $child_url : $child_dir).'/'.saveo_check_min_file($file, $child_dir);
		else if (file_exists(($theme_dir).'/'.($file)))
			$dir = ($return_url ? $theme_url : $theme_dir).'/'.saveo_check_min_file($file, $theme_dir);
		return $dir;
	}
}

if (!function_exists('saveo_get_file_url')) {	
	function saveo_get_file_url($file) {
		return saveo_get_file_dir($file, true);
	}
}

// Return file extension from full name/path
if (!function_exists('saveo_get_file_ext')) {	
	function saveo_get_file_ext($file) {
		$parts = pathinfo($file);
...
if (!function_exists('saveo_get_file_name')) {	
	function saveo_get_file_name($file, $without_ext=true) {
		$parts = pathinfo($file);
...
if (!function_exists('saveo_get_folder_dir')) {	
	function saveo_get_folder_dir($folder, $return_url=false) {
...
if (!function_exists('saveo_get_folder_url')) {	
	function saveo_get_folder_url($folder) {
		return saveo_get_folder_dir($folder, true);
	}
}


// Remove protocol part from URL
if (!function_exists('saveo_remove_protocol_from_url')) {
	function saveo_remove_protocol_from_url($url) {
...
if (!function_exists('saveo_add_to_url')) {
	function saveo_add_to_url($url, $prm) {
		if (is_array($prm) && count($prm) > 0) {
