...
if (!function_exists('saveo_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'saveo_hovers_theme_setup3', 3 );
	function saveo_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		saveo_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'saveo'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'saveo') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'saveo'),
					'slide_left'	=> esc_html__('Slide from Left',	'saveo'),
					'slide_right'	=> esc_html__('Slide from Right',	'saveo'),
					'slide_top'		=> esc_html__('Slide from Top',		'saveo'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'saveo'),
//					'arrow'			=> esc_html__('Arrow',				'saveo'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'saveo'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'saveo') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'saveo'),
					'icon'	=> esc_html__('Icon',	'saveo'),
					'icons'	=> esc_html__('Icons',	'saveo'),
					'zoom'	=> esc_html__('Zoom',	'saveo'),
					'fade'	=> esc_html__('Fade',	'saveo'),
					'slide'	=> esc_html__('Slide',	'saveo'),
					'pull'	=> esc_html__('Pull',	'saveo'),
					'border'=> esc_html__('Border',	'saveo')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('saveo_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'saveo_hovers_theme_setup9', 9 );
	function saveo_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'saveo_hovers_frontend_scripts', 1010 );
		add_filter( 'saveo_filter_localize_script','saveo_hovers_localize_script' );
		add_filter( 'saveo_filter_merge_scripts',	'saveo_hovers_merge_scripts' );
		add_filter( 'saveo_filter_merge_styles',	'saveo_hovers_merge_styles' );
		add_filter( 'saveo_filter_get_css', 		'saveo_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'saveo_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'saveo_hovers_frontend_scripts', 1010 );
	function saveo_hovers_frontend_scripts() {
		if ( saveo_is_on(saveo_get_theme_option('debug_mode')) && saveo_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'saveo-hovers', saveo_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( saveo_is_on(saveo_get_theme_option('debug_mode')) && saveo_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'saveo-hovers',  saveo_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('saveo_hovers_merge_scripts')) {
	//Handler of the add_filter( 'saveo_filter_merge_scripts', 'saveo_hovers_merge_scripts' );
	function saveo_hovers_merge_scripts($list) {
...
if (!function_exists('saveo_hovers_merge_styles')) {
	//Handler of the add_filter( 'saveo_filter_merge_styles', 'saveo_hovers_merge_styles' );
	function saveo_hovers_merge_styles($list) {
...
if (!function_exists('saveo_hovers_localize_script')) {
	//Handler of the add_filter( 'saveo_filter_localize_script','saveo_hovers_localize_script' );
	function saveo_hovers_localize_script($arr) {
		$arr['button_hover'] = saveo_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('saveo_hovers_add_icons') ) {
	function saveo_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'saveo') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'saveo');
						else
							esc_html_e('Details', 'saveo');
					}
				?></a>
			</div>
			<?php
...
//			?><!--<div class="icons"><a href="--><?php //the_permalink(); ?><!--" aria-hidden="true" class="icon-zoom"></a></div>--><?php
...
// Add styles into CSS
if ( !function_exists( 'saveo_hovers_get_css' ) ) {
	//Handler of the add_filter( 'saveo_filter_get_css', 'saveo_hovers_get_css', 10, 4 );
	function saveo_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
/* Slide */
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link']} !important; }
.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link']} !important; }
...
.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bd_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['text_hover']} 50%, {$colors['bg_color']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_bd_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_hover']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_bd_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_hover']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_bd_color']} !important; }
