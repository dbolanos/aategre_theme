...
if ( !function_exists('saveo_need_page_title') ) {
	function saveo_need_page_title() {
		return  apply_filters('saveo_filter_need_page_title', true);
	}
}
...
if ( !function_exists('saveo_show_layout') ) {
	function saveo_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('saveo_get_logo_image') ) {
	function saveo_get_logo_image($type='') {
		$logo_image = '';
		if (saveo_get_retina_multiplier(2) > 1)
			$logo_image = saveo_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = saveo_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('saveo_get_header_video') ) {
	function saveo_get_header_video() {
		$video = '';
		if (apply_filters('saveo_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (saveo_check_theme_option('header_video')) {
				$video = saveo_get_theme_option('header_video');
...
if ( !function_exists('saveo_show_post_meta') ) {
	function saveo_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['date'])) {		// && !is_attachment() && !is_page() ) {
				$dt = apply_filters('saveo_filter_get_post_date', saveo_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			saveo_show_layout(saveo_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				saveo_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'saveo'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'saveo' ), '<span class="post_meta_item post_edit">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('saveo_show_post_featured') ) {
	function saveo_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => saveo_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => saveo_is_on(saveo_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : saveo_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? saveo_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = saveo_get_post_image();
				if (!empty($image)) 
					$image = saveo_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . saveo_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$saveo_attr = saveo_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						saveo_hovers_add_icons($args['hover']);
					}
					...
					if (($output=saveo_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						saveo_show_layout($output);
					...
					if (!empty($image)) {
						...
						saveo_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					$video = saveo_get_post_video('', false);
					if (empty($video))
						$video = saveo_get_post_iframe('', false);
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = saveo_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								saveo_show_layout($video);
							}
						?></div><?php
					}
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = saveo_get_post_audio('', false);
					if (empty($audio))
						$audio = saveo_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = saveo_get_post_audio($audio);
						...
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = saveo_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = saveo_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !saveo_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php saveo_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !saveo_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php saveo_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							saveo_show_layout($audio); 
						?></div><?php
					}
				}
				
				// Put optional info block over the thumb
				saveo_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				saveo_show_layout($post_info);
...
if ( !function_exists('saveo_get_no_image') ) {
	function saveo_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = saveo_get_theme_option( 'no_image' );
			if (empty($img)) $img = saveo_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('saveo_add_bg_in_post_nav') ) {
	function saveo_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = saveo_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), saveo_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), saveo_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'saveo-main', $css );
	}
}

// Show related posts
if ( !function_exists('saveo_show_related_posts') ) {
	function saveo_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'saveo');
				?></h3>
...
if ( !function_exists('saveo_show_portfolio_posts') ) {
	function saveo_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? saveo_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = saveo_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = saveo_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the saveo_ajax_get_posts action
if ( !function_exists( 'saveo_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_saveo_ajax_get_posts',			'saveo_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_saveo_ajax_get_posts',	'saveo_ajax_get_posts_callback');
	function saveo_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			saveo_storage_set('blog_mode', 'blog');
			saveo_storage_set('options_meta', get_post_meta($id, 'saveo_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => saveo_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'saveo');
		}
...
// Show pagination
if ( !function_exists('saveo_show_pagination') ) {
	function saveo_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = saveo_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'saveo' ),
				'next_text' => esc_html__( '>', 'saveo' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'saveo' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'saveo'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'saveo') : esc_html__('Newest posts', 'saveo') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'saveo') : esc_html__('Older posts', 'saveo'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
